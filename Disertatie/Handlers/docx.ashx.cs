...
namespace Disertatie.Handlers
{
    ...
    /// </summary>
    public class docx : IHttpHandler, IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            Stream docxStream = cloudConsumer.GetDocument(context.Request["fileId"]);
            
            byte[] buffer = new byte[16*1024];
            MemoryStream ms = new MemoryStream();
            
            int read;
            while ((read = docxStream.Read(buffer, 0, buffer.Length)) > 0)
            {
            ...
            try
            {
                ...
                string header = string.Format(headerTemplate, "file", "preview.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document");
