    ...
    public class MsOnlineClaimsHelper
    {
            ...
            if (_cachedCookieContainer == null || DateTime.Now > _expires)
            {
                ...
                if (!string.IsNullOrEmpty(cookies.FedAuth))
                {
                    ...
                    if (_useRtfa)
                    {
                        ...
                        catch (Exception) { Debug.WriteLine("rtFA cookie missing!"); }
                    }
        ...
        private MsoCookies getSamlToken()
        {
            ...
            try
            {
                ...
                if (String.IsNullOrEmpty(KnownSecurityToken)) // when the user password is known
                {
                    //get token from STS
                ...
                else
                {
                    ...
                    ret.Expires = Convert.ToDateTime(DateTime.Now.AddDays(1));
                    data = Encoding.UTF8.GetBytes(KnownSecurityToken);
                }
                        ...
                        if (webResponse.StatusCode == HttpStatusCode.MovedPermanently)
                        {
                            ...
                            using (Stream stream2 = request2.GetRequestStream())
                            {
                                ...
                                using (HttpWebResponse webResponse2 = request2.GetResponse() as HttpWebResponse)
                                {
                                    ...
                                    ret.rtFa = (webResponse.Cookies["rtFa"] != null) ? webResponse.Cookies["rtFa"].Value : null;
